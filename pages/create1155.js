...
export default function CreateItem() {
  ...
  async function createMarket() {
    ...
    const { name , price , amount } = formInput
    ...
    try {
      ...
      const url = `https://ipfs.infura.io/ipfs/${added.path}`
      console.log(url)
      const sig = await sign(tokenid , price , url , amount)
      console.log(sig)
...
async function sign(TokenId , Minprice , Uri ,amount ) {
    let tokenId = TokenId
    ...
    const provider = new ethers.providers.Web3Provider(connection)
        ...
        const signature = await signer._signTypedData(
          // Domain
          {
            ...
            version: '1.0.0',
            chainId: 4,
            verifyingContract: "0x4B01E162587F30a962E992332063F36ec86eE391",
          },
          // Types
          {
            NFTVoucher: [
