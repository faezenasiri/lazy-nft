...
export default function CreateItem() {
  ...
  const [formInput, updateFormInput] = useState({ price: '', name: '',amount: '',royaltyPercentage:''})
  ...
  async function createMarket() {
     ...
     const { name , price , amount,royaltyPercentage } = formInput
    ...
    try {
      ...
      const url = `https://ipfs.infura.io/ipfs/${added.path}`
      console.log(url)
      const sig = await sign(tokenid , price , url , amount,royaltyPercentage)
      console.log(sig)
...
async function sign(TokenId , Minprice , Uri ,amount,royaltyPercentage ) {
    let tokenId = TokenId
  ...
  return (
    <div className="flex justify-center">
      <div className="w-1/2 flex flex-col pb-12">
        ...
        <input
          placeholder="royaltyPercentage"
          className="mt-2 border rounded p-4"
          onChange={e => updateFormInput({ ...formInput, royaltyPercentage: e.target.value })}
        />
        <input
          type="file"
          name="Asset"
